...
// ON1ARF
   { "VoxThresholdKickin", "%d", CLASS_VAR(VoxThresholdKickin,0)},
...
// ON1ARF
   { "AudioDevice2", "%s", CLASS_VAR(AudioDevice2,0)},
// ON1ARF
   { "AudioDeviceType", "%d", CLASS_VAR(AudioDeviceType,0)},
// ON1ARF
   { "AudioOutMuted", "%d", CLASS_VAR(AudioOutMuted,0)},
...
fprintf(stderr,"CreatePort = %s, x = %d \n",Arg,x);
...
sleep (3);
fprintf(stderr,"before: Arg = *%s* \n",Arg);
fprintf(stderr,"p_pPort = %X \n",pPort);
fprintf(stderr,"addr pPort->NodeName = %X \n",&pPort->NodeName);
sleep (3);
	char * zzz;

//			zzz = strdup(Arg);

fprintf(stderr,"zzz  = %X \n",zzz);
zzz=NULL;
fprintf(stderr,"zzz = %X,  strlen(arg)  = %d \n",zzz,strlen(Arg));
zzz = (char *)malloc(strlen(Arg)+1);
fprintf(stderr,"zzz  = %X \n",zzz);
if (zzz) {
fprintf(stderr,"if zzz  = %s \n",zzz);
	strcpy(zzz,Arg);
}; // end if
