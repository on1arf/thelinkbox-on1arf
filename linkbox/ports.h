/* Copyright (C) 2002 GFRN systems
   ...
   $Log: ports.h,v $
	...
	add AudioDevice2: 2nd AudioDevice for output for FIFO device
	Add AudioOutMuted

	// 2013/10/20 ON1ARF
	Added option VoxThresholdKickin: A "kicking" value to increase the VoxThreshold at
	the beginning of a stream

	// 2013/10/10 ON1ARF
   Added option "AudiodeviceIsFifo": read-only audio device, reads raw stream from file
			(or FIFO-device), no DSP_SND commands
...
struct pashared_str {
	...
	int i_numchannel, o_numchannel; // mono or stereo?
	int p_state; // 0 = waiting, 1 = playback
	int c_bufferfull; // flags
...
// receiver audio ring buffer
   ...
   void Write8754(char Data);
	...
	// DeviceIsFido = Device is FIFO device, read raw PCM, no SND_DSP functions, readonly (no write)
