...
int AccessAsteriskEnable(struct config_entry *p, char *Arg, AfCmd Cmd, int x);
int AccessAsteriskDesc(struct config_entry *p, char *Arg, AfCmd Cmd, int x);
...
// Optional parameters
   ...
   { "AsteriskPort", "%d", &AsteriskPort,0 },
   { "AsteriskBind2IP", "%s", &AsteriskBind2IP, 0},
   { "AsteriskIP", "%s", &AsteriskIP, 0},
   { "AsteriskDesc", "%F", &AsteriskDesc, 0, AccessAsteriskDesc},
   { "AsteriskEnable", "%F", &AsteriskEnable, 0, AccessAsteriskEnable},
...
int AccessAsteriskEnable(struct config_entry *p, char *Arg, AfCmd Cmd, int x)
{
   ...
   switch(Cmd) {
      case AF_SET_VAR:
         if(sscanf(Arg,"%d",&AsteriskEnable) != 1) {
            Ret = ERR_CONFIG_FILE;
         }
         else if(AsteriskEnable == 0) {
         // disabled, "disconnect" Asterisk client if there is one
            AsteriskDisconnect(NULL);
         }
         else if(ConfigPass > 1 && AsteriskPort == 0) {
            LOG_ERROR(("Error: Can't enable Asterisk, AsteriskPort isn't set\n"));
            AsteriskEnable = FALSE;
         }
         break;
      ...
      case AF_DISPLAY_VAR:
         snprintf(Arg,x,"%d",AsteriskEnable);
         break;
   }
...
int AccessAsteriskDesc(struct config_entry *p, char *Arg, AfCmd Cmd, int x)
{
   ...
   switch(Cmd) {
      case AF_SET_VAR:
         if(AsteriskDesc != NULL) {
            free(AsteriskDesc);
         }
         AsteriskDesc = strdup(Arg);
         break;
      ...
      case AF_SET_DEFAULT:
         ...
         AsteriskDesc = strdup("Asterisk");
         break;

      case AF_DISPLAY_VAR:
         snprintf(Arg,x,"%s",AsteriskDesc);
         break;
   }
